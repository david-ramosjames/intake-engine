  ...
  return (
    <div className="mx-auto max-w-3xl px-8 py-10">
      ...
      <p className="mt-1 text-sm text-white/50">
        Event-driven workflows for {org?.name ?? "your organization"}: when something happens, run
        actions.
      </p>

      <div className="mt-8 rounded-2xl border border-dashed border-white/15 p-8 text-center">
        <p className="text-white/60">No automations yet.</p>
        ...
        <button
          disabled
          className="mt-5 cursor-not-allowed rounded-full border border-white/15 px-5 py-2.5 text-sm text-white/40"
        >
      ...
      <div className="mt-8">
        <h2 className="text-sm font-medium text-white/70">Trigger → actions</h2>
        <div className="mt-3 rounded-2xl border border-white/10 bg-white/[0.02] p-5 text-sm">
          <div className="text-white/60">
            <span className="rounded-md bg-white/10 px-2 py-1 font-mono text-xs">LEAD_COMPLETED</span>{" "}
            <span className="text-white/40">then</span>
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {ACTIONS.map((a) => (
              <span key={a} className="rounded-full border border-white/10 px-3 py-1 text-white/60">
                {a}
              </span>
            ))}
          </div>
